...
describe("buildBookingPayload", () => {
  it("includes contact, appointment, and survey sections", () => {
    ...
    expect(payload).toContain("Nutritional Counseling");
...
describe("buildBookingUrl", () => {
  it("appends a service query param and book hash", () => {
    const url = buildBookingUrl("Nutritional Counseling");
    expect(url).toBe(
      `${CLINIC_CONFIG.productionUrl}?service=Nutritional%20Counseling#book`,
    );
  });
